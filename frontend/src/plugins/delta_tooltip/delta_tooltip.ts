...
} from './crosshair_line_pane.ts';
import {
    DeltaSingleTooltipData,
    DeltaTooltipData,
    DeltaTooltipPaneView,
} from './delta_tooltip_pane.ts';
import {
    MultiTouchChartEvents,
    MultiTouchInteraction,
} from './multi_touch_chart_events.ts';
    ...
    priceExtractor: (data: LineData | CandlestickData | WhitespaceData) => {
        if ((data as LineData).value !== undefined) {
            return [
                ...
                (data as LineData).value.toFixed(2),
            ];
        }
...
export class DeltaTooltipPrimitive implements ISeriesPrimitive<Time> {
    ...
    private _showTooltip(interactions: MultiTouchInteraction) {
        ...
        for (let i = 0; i < Math.min(2, interactions.points.length); i++) {
            ...
            if (data) {
                const [priceValue, priceString] =
                    this._options.priceExtractor(data);
        ...
        if (priceValues.length > 1) {
            ...
            const firstPrice = correctOrder
                ? priceValues[0][0]
                : priceValues[1][0];
            const secondPrice = correctOrder
                ? priceValues[1][0]
                : priceValues[0][0];
