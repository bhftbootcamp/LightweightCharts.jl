...
import { TooltipElement, TooltipOptions } from './tooltip_element.ts';
import { fixDateShift, formattedDateAndTime } from '../../helpers/time.ts';
...
export class TooltipPrimitive implements ISeriesPrimitive<Time> {
    ...
    constructor(dateShift: number, dateScale: number, options: Partial<TooltipPrimitiveOptions>) {
        this._options = {
            ...defaultOptions,
            ...options,
        };
    ...
    private _onMouseMove(param: MouseEventParams) {
        ...
        const [date, time] = formattedDateAndTime(
            data.time ? fixDateShift(data.time as number, this._dateShift, this._dateScale).date.valueOf() : undefined
        )
        if (this._tooltip) {
            const tooltipOptions = this._tooltip.options();
            const topMargin = tooltipOptions.followMode == 0 ? tooltipOptions.topOffset + 10 : 0;
            this.setData({
                x: coordinate ?? 0,
